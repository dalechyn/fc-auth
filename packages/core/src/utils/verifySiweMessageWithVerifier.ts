import { SiweError, type SiweMessage, type SiweResponse } from 'siwe'
...
export type VerifySiweMessageWithVerifierReturnType = Omit<
  SiweResponse,
  'error'
> &
  FarcasterResourceParameters

const voidVerifyFid = (_custody: Address) =>
  Promise.reject(new Error('Not implemented: Must provide an fid verifier'))
...
export const verifySiweMessageWithVerifier = async (
  ...
  const siweResponse = await verifySignature(
    validatedMessage,
    signature,
    provider,
  )
  ...
  if (!siweResponseWithResources.success) {
    const errMessage =
      siweResponseWithResources.error?.type ?? 'Failed to verify SIWE message'
    throw new AuthClientError('unauthorized', errMessage)
  }

  const siweResponseWithResourcesAndValidatedFid = await verifyFidOwner(
    siweResponseWithResources,
    getFid,
  )
  if (!siweResponseWithResourcesAndValidatedFid.success) {
    const errMessage =
      siweResponseWithResourcesAndValidatedFid.error?.type ??
      'Failed to validate fid owner'
    throw new AuthClientError('unauthorized', errMessage)
  }
...
): SiweResponse & FarcasterResourceParameters => {
